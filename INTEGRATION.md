...
# MBEventHandling Integration Notes

This document describes how to integrate MBEventHandling into your application.

MBEventHandling is designed as a *universal framework* with support for the following platforms within a single binary:
...
MBEventHandling **requires Xcode 9.0** to compile.

### Options for integration

There are two supported options for integration:

- **[Carthage integration](#carthage-integration)** uses the [Carthage](https://github.com/Carthage/Carthage) dependency manager to add MBEventHandling to your project.

- **[Manual integration](#manual-integration)** involves embedding the `MBEventHandling.xcodeproj` file within your project’s Xcode workspace.

Once properly integrated, you can make use of [the API](https://rawgit.com/emaloney/MBEventHandling/master/Documentation/API/index.html) provided by MBEventHandling in any Objective-C file using the statement:
...
### 1. Add MBEventHandling to Cartfile
...
To integrate MBEventHandling in your workspace, you would start by adding the following line to the `Cartfile`:

```
github "emaloney/MBEventHandling" ~> 3.1.0
```

This specifies that Carthage use the latest version of MBEventHandling that is API-compatible with 3.1.0, i.e. any 3.1.*x* version.

**Note:** Be sure to check the [latest releases](https://github.com/emaloney/MBEventHandling/releases); there may be a newer version than 3.1.0 that is no longer API-compatible.

### 2. Download MBEventHandling using Carthage

Once added to your `Cartfile`, you can use Carthage to download MBEventHandling to your machine:
...
### 3. Build MBEventHandling using Carthage

To have Carthage build (or re-build) MBEventHandling, issue the command:

```
carthage build MBEventHandling
```
...
Even though MBEventHandling is designed as a universal framework, during the build process, Carthage splits the framework into separate binaries for each Apple platform.

After a successful build, you will find platform-specific binaries for `MBEventHandling.xcodeproj` in the appropriate Carthage build folder:
...
Next, select the Build Target to which you want to add `MBEventHandling.framework`, and then select the General tab.
...
Manual integration involves embedding the Xcode project file for MBEventHandling directly within your own Xcode workspace.

Successful manual integration depends on the particulars of your project structure and development workflow.

> **Note:** These instructions assume that you are using an Xcode workspace specifically—and not just a project file—in order to integrate MBEventHandling.
...
```
Carthage/Checkouts/MBEventHandling/MBEventHandling.xcodeproj
```

### Other Integrations

If you acquired the MBEventHandling’s source code through some other means, you will need to locate the `MBEventHandling.xcodeproj` project file: .
...
Want to learn more about MBEventHandling? Check out [the README](https://github.com/emaloney/MBEventHandling/blob/master/README.md) or [the API documentation](https://rawgit.com/emaloney/MBEventHandling/master/Documentation/API/index.html).
